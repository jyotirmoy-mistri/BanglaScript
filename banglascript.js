...
function tokenize(code) {
    ...
    while (current < code.length) {
        ...
        if (/[\u0980-\u09FFa-zA-Z_]/.test(char)) {
            ...
            // সমস্ত হারানো কিওয়ার্ড রিস্টোর করা হলো এবং 'পপআপ_খুলুন' যুক্ত করা হলো
            let keywords = [
                ...
                'তারিখ_বাছাই_করো', 'রং_বাছাই_করো', 'লাইভ_ঘড়ি_চালু', 'কনসোলে_লেখো', 'ফাইল_যুক্ত_করো', 'পপআপ_খুলুন'
            ];
...
async function runTokens(tokens, variables, functions, outputBox) {
    ...
    while(i < tokens.length) {
        ...
        if (token.type === 'কীওয়ার্ড' && token.value === 'ধরি') {
            ...
            // ⚠️ ১০০% ফিক্সড: JSON এবং Array পার্সিং লজিক (টেবিল আর ভাঙবে না!)
            if (val1_token === '{' || val1_token === '[') {
                let braceCount = 0; let bracketCount = 0; let k = i + 3; let structTokens = [];
                while (k < tokens.length) {
                    let t = tokens[k];
                    if (t.value === '{') braceCount++; if (t.value === '}') braceCount--;
                    if (t.value === '[') bracketCount++; if (t.value === ']') bracketCount--;
                    structTokens.push(t);
                ...
                let jsonStr = structTokens.map((t, idx) => {
                    if (t.type === 'টেক্সট') return '"' + t.value.replace(/"/g, '\\"') + '"';
                    let nextT = structTokens[idx+1];
                    if (t.type === 'চলক' && nextT && nextT.value === ':') return '"' + t.value + '"';
                    ...
                    if (t.type === 'সংখ্যা') return banglaToEnglishNum(t.value);
                    return t.value;
                }).join(' ');

                try { variables[varName] = JSON.parse(jsonStr); } 
                catch(e1) { 
                    try { variables[varName] = new Function('return ' + jsonStr)(); } 
                    catch(e2) { variables[varName] = jsonStr; } 
                }
        ...
        // ⚠️ হারানো কমান্ড রিস্টোর: ওয়েবসাইট বানাও (HTML Add)
        else if (token.type === 'কীওয়ার্ড' && token.value === 'ওয়েবসাইট_বানাও') {
            let html = tokens[i+1] ? (tokens[i+1].type === 'টেক্সট' ? tokens[i+1].value : (variables[tokens[i+1].value] !== undefined ? variables[tokens[i+1].value] : tokens[i+1].value)) : '';
            if(outputBox) outputBox.insertAdjacentHTML('beforeend', `<div class="bengali-ui-element">${html}</div>`);
            let j = i + 2; while(j < tokens.length && tokens[j].value !== ';') { j++; } i = j + 1;
        }
        // ⚠️ হারানো কমান্ড রিস্টোর: রং বদলানো
        else if (token.type === 'কীওয়ার্ড' && token.value === 'রং_বদলানো') {
            let id = tokens[i+1].value;
            let color = "";
            let k = i + 2;
            if(tokens[k] && tokens[k].value === '=') k++;
            color = tokens[k] ? (tokens[k].type === 'টেক্সট' ? tokens[k].value : (variables[tokens[k].value] || tokens[k].value)) : '';
            
            let actualColor = color === 'লাল'?'red':color==='সবুজ'?'green':color==='নীল'?'blue':color==='সাদা'?'white':color==='কালো'?'black':color==='হলুদ'?'yellow':color;
            let el = document.getElementById(id); if(el) el.style.backgroundColor = actualColor;
            let j = k + 1; while(j < tokens.length && tokens[j].value !== ';') { j++; } i = j + 1;
        }
        // ⚠️ হারানো কমান্ড রিস্টোর: লেখা বদলানো
        else if (token.type === 'কীওয়ার্ড' && token.value === 'লেখা_বদলানো') {
            let id = tokens[i+1].value, actualText = "";
            let k = i + 2;
            if(tokens[k] && tokens[k].value === '=') k++;
            
            if (tokens[k+1] && tokens[k+1].value === '[') {
                let idxStr = tokens[k+2].value; let idx = tokens[k+2].type === 'টেক্সট' ? idxStr : (isNaN(parseInt(banglaToEnglishNum(idxStr))) ? idxStr : parseInt(banglaToEnglishNum(idxStr)));
                let arrOrObj = variables[tokens[k].value]; actualText = arrOrObj ? arrOrObj[idx] : '';
                if (typeof actualText === 'object') actualText = JSON.stringify(actualText); k += 5; 
            } else {
                actualText = tokens[k] ? (tokens[k].type === 'টেক্সট' ? tokens[k].value : (variables[tokens[k].value] !== undefined ? variables[tokens[k].value] : tokens[k].value)) : '';
                if (Array.isArray(actualText)) actualText = actualText.join(", "); else if (typeof actualText === 'object' && actualText !== null) actualText = JSON.stringify(actualText);
                k++;
            }
            let el = document.getElementById(id); if(el) el.innerHTML = actualText;
            let j = k; while(j < tokens.length && tokens[j].value !== ';') { j++; } i = j + 1;
        }
        // ⚠️ হারানো কমান্ড রিস্টোর: অ্যানিমেশন দাও
        else if (token.type === 'কীওয়ার্ড' && token.value === 'অ্যানিমেশন_দাও') {
            let id = tokens[i+1].value;
            let anim = tokens[i+2] ? (tokens[i+2].type === 'টেক্সট' ? tokens[i+2].value : (variables[tokens[i+2].value] || tokens[i+2].value)) : '';
            let el = document.getElementById(id);
            if(el) { el.style.animation = anim === 'ঘোরাও' ? 'spin 2s linear infinite' : anim === 'লাফাও' ? 'bounce 1s infinite' : anim === 'কাঁপাও' ? 'shake 0.5s infinite' : anim; }
            let j = i + 3; while(j < tokens.length && tokens[j].value !== ';') { j++; } i = j + 1;
        }
        // ⚠️ হারানো কমান্ড রিস্টোর: বার্তা দাও
        else if (token.type === 'কীওয়ার্ড' && token.value === 'বার্তা_দাও') {
            let alertValue = tokens[i+1] ? (tokens[i+1].type === 'টেক্সট' ? tokens[i+1].value : (variables[tokens[i+1].value] !== undefined ? variables[tokens[i+1].value] : tokens[i+1].value)) : '';
            alert(alertValue); let j = i + 2; while(j < tokens.length && tokens[j].value !== ';') { j++; } i = j + 1;
        }
        // ⚠️ হারানো কমান্ড রিস্টোর: মেমরি এবং স্ক্রিন
        else if (token.type === 'কীওয়ার্ড' && token.value === 'স্ক্রিন_পরিষ্কার') {
            if(outputBox) outputBox.innerHTML = '';
            let j = i + 1; while(j < tokens.length && tokens[j].value !== ';') { j++; } i = j + 1;
        }
        else if (token.type === 'কীওয়ার্ড' && token.value === 'সেভ_করো') {
            let key = tokens[i+1] ? (tokens[i+1].type === 'টেক্সট' ? tokens[i+1].value : (variables[tokens[i+1].value] || tokens[i+1].value)) : '';
            let val = tokens[i+3] ? (tokens[i+3].type === 'টেক্সট' ? tokens[i+3].value : (variables[tokens[i+3].value] !== undefined ? variables[tokens[i+3].value] : tokens[i+3].value)) : '';
            if(typeof val === 'object') val = JSON.stringify(val); localStorage.setItem(key, val);
            let j = i + 4; while(j < tokens.length && tokens[j].value !== ';') { j++; } i = j + 1;
        }
        else if (token.type === 'কীওয়ার্ড' && token.value === 'মেমরি_থেকে_আনো') {
            let varName = tokens[i+1].value;
            let key = tokens[i+3] ? (tokens[i+3].type === 'টেক্সট' ? tokens[i+3].value : (variables[tokens[i+3].value] || tokens[i+3].value)) : '';
            let data = localStorage.getItem(key); try { data = JSON.parse(data); } catch(e){} variables[varName] = data || '';
            let j = i + 4; while(j < tokens.length && tokens[j].value !== ';') { j++; } i = j + 1;
        }
        // নতুন ফিচার: পপআপ খুলুন (Modal Dialog)
        else if (token.type === 'কীওয়ার্ড' && token.value === 'পপআপ_খুলুন') {
            let content = tokens[i+1] ? (tokens[i+1].type === 'টেক্সট' ? tokens[i+1].value : (variables[tokens[i+1].value] || tokens[i+1].value)) : '';
            let modal = document.createElement('div');
            modal.style = "position:fixed; top:50%; left:50%; transform:translate(-50%, -50%); background:#fff; color:#000; padding:25px; border-radius:15px; box-shadow:0 0 20px rgba(0,0,0,0.6); z-index:9999; text-align:center; min-width: 250px;";
            modal.innerHTML = content + `<br><button onclick="this.parentElement.remove()" style="margin-top:20px; padding:10px 20px; background:#ff4757; color:white; font-size:16px; border:none; border-radius:5px; cursor:pointer; font-weight:bold;">বন্ধ করুন</button>`;
            ...
            try {
                ...
                } else { console.error("ফাইল পাওয়া যায়নি: " + fileName); }
            ...
            if (Array.isArray(dataArr) && dataArr.length > 0) {
                let tableHTML = `<table id="${id}" style="width:100%; border-collapse: collapse; margin: 15px 0; font-family: sans-serif; text-align: left; box-shadow: 0 4px 8px rgba(0,0,0,0.1); background: white; border-radius: 8px; overflow: hidden;">`;
                tableHTML += `<tr style="background-color: #4CAF50; color: white;">`;
                Object.keys(dataArr[0]).forEach(key => { tableHTML += `<th style="padding: 12px; border-bottom: 1px solid #ddd;">${key}</th>`; });
                tableHTML += `</tr>`;
                
                dataArr.forEach((row, index) => {
                    let bg = index % 2 === 0 ? '#f9f9f9' : '#ffffff';
                    tableHTML += `<tr style="background-color: ${bg}; color: #333;">`;
                    Object.values(row).forEach(val => { tableHTML += `<td style="padding: 12px; border-bottom: 1px solid #ddd;">${val}</td>`; });
                    tableHTML += `</tr>`;
                });
                tableHTML += `</table>`;
        ...
        else if (token.type === 'কীওয়ার্ড' && token.value === 'ক্লিক_করলে') {
            let id = tokens[i+1].value;
            let j = i + 2; if (tokens[j] && tokens[j].value === '{') j++;
            let blockStart = j, blockEnd = j, braceCount = 1;
            while (braceCount > 0 && blockEnd < tokens.length) {
            ...
            let el = document.getElementById(id);
            if (el) { el.addEventListener('click', async function() { await runTokens(blockTokens, variables, functions, outputBox); }); }
            i = blockEnd; 
        }
...
/** * ==========================================
 * পার্ট ৪: মূল এক্সিকিউটর
 * ==========================================
 */
async function executeBengaliCode() {
    let htmlCode = document.getElementById('html-editor') ? document.getElementById('html-editor').value : "";
    let cssCode = document.getElementById('css-editor') ? document.getElementById('css-editor').value : "";
    let banglaCode = document.getElementById('bangla-editor') ? document.getElementById('bangla-editor').value : document.getElementById('editor').value;
    ...
    outputBox.innerHTML = htmlCode ? htmlCode : '';

    let oldStyles = document.querySelectorAll('.dynamic-user-style');
    oldStyles.forEach(s => s.remove());
    if(cssCode) {
        let styleTag = document.createElement('style');
        styleTag.innerHTML = cssCode;
        styleTag.className = "dynamic-user-style";
        document.head.appendChild(styleTag);
    }
    
    if (window.bengaliAudio) { window.bengaliAudio.pause(); }
    if(window.bengaliStreams) {
        window.bengaliStreams.forEach(stream => { stream.getTracks().forEach(track => track.stop()); });
        window.bengaliStreams = [];
    }

    if(window.bengaliIntervals) window.bengaliIntervals.forEach(id => clearInterval(id));
    window.bengaliIntervals = [];

    if(window.bengaliTimeouts) window.bengaliTimeouts.forEach(id => clearTimeout(id));
    window.bengaliTimeouts = [];

    if(window.bengaliKeyListeners) window.bengaliKeyListeners.forEach(fn => window.removeEventListener('keydown', fn));
    window.bengaliKeyListeners = [];

    try {
        let tokens = tokenize(banglaCode);
        let variables = {}; 
        let functions = {}; 
        await runTokens(tokens, variables, functions, outputBox); 
    } catch (error) {
        console.error(error);
        outputBox.innerHTML += "<br><span style='color:red;'>কোডে কোনো ভুল আছে!</span>";
    }
}
